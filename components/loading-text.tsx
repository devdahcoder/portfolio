...
const LoadingText: (props: Props) => JSX.Element = (props: Props) => {
    return (
        <div className="text-center z-50 flex flex-col items-center justify-center text-dark-bg dark:text-white 
            ...
            <div className="text-4xl small-xs:text-6xl small-sm:text-8xl font-seibold capitalize">
                <p className={`${!props?.isTextCompleted && 'animate-blinker'}`} ref={props?.loadingTextRefElement}></p>
            </div>

            <div className={`relative text-xs small-sm:text-xl
                flex flex-col small-xs:flex-row items-center justify-center text-center font-inter font-medium space-x-7 
                small-xs:before:contents-[] small-xs:before:absolute small-xs:before:right-28 small-sm:before:right-44 lg:before:right-0 small-xs:before:top-1/2 small-xs:before:h-4 small-xs:before:rounded-full
